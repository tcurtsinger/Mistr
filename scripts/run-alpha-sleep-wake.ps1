param(
    [switch]$SkipBuild,
    [ValidateRange(5, 60)][int]$WaitMinutes = 20,
    [int]$Port = 9342
)
...
try {
    ...
    try {
        ...
        Remove-Item Env:MISTR_SLEEP_WAKE_WAIT_MINUTES -ErrorAction SilentlyContinue
    }
}
finally {
    Pop-Location
