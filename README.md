...
Reflectivity keeps the operational NOAA/NWS RGB thresholds while using a display-only weak-return visibility curve: non-positive dBZ is transparent, positive returns grow progressively to full opacity at 20 dBZ, and native measured dBZ remains available to inspection. Neutral coastlines, major roads, state boundaries, and important labels stay legible above radar without globally lowering radar opacity or adding another map provider. This presentation reduces clear-air clutter visually; it does not claim meteorological clutter classification. See [Radar Rendering Quality](docs/25_RADAR_RENDERING_QUALITY.md).
