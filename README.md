...
Every launch first paints the bundled pinned archive as a safe bridge, then automatically acquires current live radar for the stored site or KTLX on a fresh profile. The single searchable picker exposes the 155 operational WSR-88D sites qualified against the fixed live provider, including Alaska, Hawaii, Guam, and Puerto Rico. Test, TDWR, decommissioned, and provider-absent identifiers are excluded. Live polling then requests exact-next provider volumes and accumulates up to 20 chronological GPU-resident frames. Retained frames are reused, only the appended frame is uploaded, and the oldest is evicted at the bound. See [Bounded Rolling Live History](docs/21_BOUNDED_ROLLING_LIVE_HISTORY.md) for the exact ownership contract and [Alpha UI and Live-Site Hardening](docs/23_ALPHA_UI_AND_SITE_HARDENING.md) for the catalog and startup/status decisions.
