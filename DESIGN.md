...
**The Operational Context Rule.** Land and water remain the base plane beneath radar. Local streets, buildings, railways, water names, and secondary places also stay below radar. Only major routes, boundaries, and important place labels remain above it with enough light-and-dark contrast to survive changing radar colors. Split water polygons are never outlined as persistent context because their generated seams and generalized geometry change across zoom levels. Motorway, trunk, and primary segments use the same continuous zoom treatment rather than swapping style layers at fixed zooms. At far regional zoom, only coherent interstate and U.S.-highway networks remain visible; state and unnetworked routes fade in with the detailed road graph rather than surfacing as disconnected generalized fragments. Global radar opacity is not used to make the map visible, and context is never allowed to become a luminous wireframe.
...
The map fills the window. The top-center toolbar contains Mistr identity followed by a radar-source icon, a direct recenter icon, and an eye icon that opens the real two-state radar-view popup. The source panel begins with explicit `National` and `Site` choices; selecting `Site` reveals the canonical searchable station list, while supporting and accessible copy identifies National coverage as CONUS. The popup's visible labels are exactly `Smooth` and `Native`, with `Smooth` as the default, and the eye control exposes the tooltip `Radar View`.

The Phase 3 source control appears only because the branch can paint a complete National observation and produce an authoritative GPU receipt. Requested source remains separate from painted-source labeling; the old renderer stays visible until every replacement chunk uploads, complete viewport coverage draws, and the matching GPU fence completes. Failed or superseded work restores the prior source without changing timestamp, age, inspection identity, or persistence. This behavior is review-branch functionality until merge, not a claim about released `main`.

`Smooth` filters the spatial presentation of one measured observation. It does not interpolate between scan times, generate a frame, change decoded values, or change the dBZ returned by inspection. `Native` shows the nearest selected-site polar gate or nearest National numeric-grid cell at the active presentation level. National smoothing never bridges missing or no-coverage cells, and National interrogation always uses the exact retained base grid. Changing this display choice therefore leaves source, selected observation, measured time, numeric age, timeline position, and paint identity unchanged.
...
The source control's accessible name and tooltip follow the observation that actually painted. While National or a different site is being acquired, a compact visible acquisition indicator and notice name both the radar that remains displayed and the requested source; the toolbar never claims the switch before GPU publication succeeds.
...
The bottom-center playback bar remains stable regardless of panel state. During Site operation it contains play/pause, direct scan scrubbing, the displayed scan timestamp, numeric frame age, and the active dBZ sample. Phase 3 National presents one current observation, so transport and timeline movement are disabled without inventing history; timestamp, age, and exact dBZ remain visible. It does not display the words `Fresh`, `Stale`, `Playing`, `Paused`, or `Newest`. Green means newest for the active source, even when the newest National mosaic is older than the prior Site scan.
...
- Land and water use close matte charcoal tones so their edge reads as geography rather than a bright slab; parks and wooded areas remain secondary texture beneath radar.
...
- Country and state boundaries, motorways, primary/trunk routes, major route identifiers, states, countries, and important city labels sit above radar using cool neutral contrast rather than Stormlight color.
- Regional framing uses one continuous motorway/trunk/primary graph with a dark support stroke and restrained neutral centerline. Interstate and U.S.-highway networks remain at far zoom; state and unnetworked routes transition from hidden to visible across the detailed-source handoff. Local-road opacity rises gradually beneath radar instead of appearing at a hard zoom boundary. At detailed zooms, the major-route support stroke and label halos remain legible across dark, green, yellow, red, and weak-return backgrounds without becoming bold parallel rails. Neither treatment lowers the entire radar layer's opacity.
