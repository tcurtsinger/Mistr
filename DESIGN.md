...
National Phase 2 remains deliberately invisible. The merged coordinator recognizes typed `Site`/future `National` identities, and the active branch can acquire, decode, and transfer MRMS only through a hidden packaged diagnostic, but the toolbar remains exactly the current site, recenter, and view sequence. No placeholder National choice, disabled source toggle, MRMS status, or supporting panel ships before a complete National observation can paint. The later source control will replace the site-only entry point only in the phase that delivers working National radar and preserves the same one-panel, focus-return, compact-target, and painted-truth rules.
