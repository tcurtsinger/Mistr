...
The active National Phase 1 work is deliberately invisible. It extracts source-session ownership and introduces typed `Site`/future `National` identities, but the toolbar remains exactly the current site, recenter, and view sequence. No placeholder National choice, disabled source toggle, or supporting panel ships before a complete National observation can paint. The later source control will replace the site-only entry point only in the phase that delivers working National radar and preserves the same one-panel, focus-return, compact-target, and painted-truth rules.
