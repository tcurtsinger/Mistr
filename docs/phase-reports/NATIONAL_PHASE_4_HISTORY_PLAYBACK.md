...
The history was strictly chronological and unique. The release runner holds one evidence-only resident reservation across its initial history snapshot, oldest/newest scrubs, 1,000 transitions, and exact inspection proof so a normal newly published MRMS observation cannot make the validator compare two different valid 20-frame timelines. Oldest/newest direct scrubs and all 1,000 transitions used factor 4 and matched retained generation/observation/content identities. One persistent inspection point was re-queried across newest-to-oldest-to-newest cuts, producing three distinct request identities and accepting only the matching retained observation each time. During active playback the inspection queue never exceeded one concurrent lookup, retained only one newest pending cut, immediately settled callers for replaced cuts, and drained completely after pause. High-zoom paused selection refined to exact factor 1 while every common overview remained resident; active high-zoom playback returned atomically to and locked factor 4. Real `WEBGL_lose_context` recovery restored the selected frame first and all 20 common residents from CPU-owned bytes with zero backend activity. During final review validation, 96- and 64-row recovery bands separately produced 4.40 and 5.50 ms rejected slices. Mistr kept the 4 ms gate unchanged, reduced work to 32 rows per frame, and the unchanged final renderer passed four consecutive packaged runs at 1.70, 1.90, 1.80, and 2.10 ms; the latest run includes the queue, UI recovery, and reversible-detail accounting fixes.
