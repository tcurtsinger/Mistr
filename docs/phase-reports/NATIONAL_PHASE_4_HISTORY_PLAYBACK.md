...
- Packaged validator tests: rejection of hot-path activity, mixed high-zoom presentation quality, missing partial-history control availability, or missing/stale failed-Site restoration evidence. The release runner directly samples the visible Play button during partial backfill and requires an enabled stable-staging sample before accepting the final 20-frame history.
...
The dedicated National Phase 4 packaged gate passes across review hardening and the focused post-merge corrections, including four consecutive runs of the final 32-row renderer after identity-bound rollback settlement and upload hardening, across the primary release-runtime history, partial-backfill control availability, residency, zero-I/O playback, quality-lock, memory, context-recovery, credit-release, exact-lookup, inspection-refresh/coalescing, post-finalization recovery receipt, active-playback failed-Site restoration, and successful Site-return criteria. `npm run verify` passes 300 frontend/script tests and 134 Rust tests across the library and binaries, together with the production build, formatting, warnings-denied clippy, Rust check, documentation links, and public-repository scan. The merged National Phase 2/3 packaged gates and unchanged selected-site Phase 4/5/6 packaged regressions also pass.
