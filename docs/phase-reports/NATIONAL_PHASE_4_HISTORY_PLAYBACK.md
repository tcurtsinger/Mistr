...
- safe National-to-Site handoff without a second timeline or transfer-credit pool.
...
`NationalHistoryState` retains immutable compressed provider objects plus complete factor-4 `PackedGrid v1` frames in chronological order under a 20-frame cap and 180 MiB backend target. It stages one current, predecessor, or newer identity at a time. Exact point lookup is bound to the painted generation, observation time, content hash, and geographic inspection identity; it re-decodes a retained object under one semaphore permit when needed.

`NationalHistoryWorkingSetController` transfers one manifest/chunk lease at a time through the existing global two-credit broker. `NationalGridLayer` indexes common and fine resources by observation identity and uploads each chunk texture in bounded row bands over animation frames under the enforced 4 ms slice. A history fence receipt is provisional: the prior residency graph and retired resources remain owned until the Rust history commit succeeds. Finalization releases them; rejection repaints the prior complete presentation.
...
| Measured history span | 37.67 minutes |
...
| Peak National GPU allocation | 65,292,096 bytes |
| Maximum upload slice | 1.30 ms |
...
The history was strictly chronological and unique. Oldest/newest direct scrubs and all 1,000 transitions used factor 4 and matched retained generation/observation/content identities. High-zoom paused selection refined to exact factor 1 while every common overview remained resident; active high-zoom playback returned atomically to and locked factor 4. Real `WEBGL_lose_context` recovery restored the selected frame first and all 20 common residents from CPU-owned bytes with zero backend activity.
...
- Rust store tests: chronology, staged rollback preservation, one-oldest eviction at 20, and the same store/snapshot model at a non-shipping 30-frame limit.
...
- Packaged validator tests: rejection of hot-path activity or mixed high-zoom presentation quality.
...
The dedicated National Phase 4 packaged gate passes the primary release-runtime history, residency, zero-I/O playback, quality-lock, memory, context-recovery, credit-release, exact-lookup, and Site-return criteria. `npm run verify` passes 264 frontend/script tests and 127 Rust tests across the library and binaries, together with the production build, formatting, warnings-denied clippy, Rust check, documentation links, and public-repository scan. The merged National Phase 2/3 packaged gates and unchanged selected-site Phase 4/5/6 packaged regressions also pass.
