...
Three passing 3840 by 2160 results across review hardening, including a final run after idempotent preparation retry:

| Measurement | Result |
|---|---:|
| Retained exact observations | 20 |
| Measured history span | 38.00 to 38.10 minutes |
...
| Highest upload slice | 1.80 ms across initial staging, detail, and both recovery passes |
...
- Rust store tests: chronology, staged rollback preservation, exact predecessor/newer stage reuse after a lost preparation response, different-kind rejection, reversible post-eviction compensation, idempotent duplicate finalization, post-newer exact-cache release, one-oldest eviction at 20, and the same store/snapshot model at a non-shipping 30-frame limit.
...
The dedicated National Phase 4 packaged gate passes three times across review hardening, including the latest exact code after idempotent preparation retry, across the primary release-runtime history, residency, zero-I/O playback, quality-lock, memory, context-recovery, credit-release, exact-lookup, inspection-refresh/coalescing, post-finalization recovery receipt, active-playback failed-Site restoration, and successful Site-return criteria. `npm run verify` passes 272 frontend/script tests and 133 Rust tests across the library and binaries, together with the production build, formatting, warnings-denied clippy, Rust check, documentation links, and public-repository scan. The merged National Phase 2/3 packaged gates and unchanged selected-site Phase 4/5/6 packaged regressions also pass.
