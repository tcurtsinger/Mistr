...
`NationalPlaybackController` always retains complete factor-4 resources, but regional play now prepares the finest complete viewport level for every retained observation before motion begins: factor 1 first and factor 2 only when factor 1 would exceed the 200 MiB target. A resident-only reservation waits for in-flight acquisition/refinement and prevents new predecessor/poll work until interaction ends. The existing complete paint remains visible during the bounded preparation; cancellation deletes partial detail. Once locked, playback changes only resident selection/draw state. After context loss, direct scrub awaits the same all-frame common-residency barrier as play; complete texture counts are insufficient until the renderer is painted, has an authoritative recovered receipt, and has no provisional mutation. Paused high-zoom selection may refine, but refinement cannot change timeline time, numeric age, source, or exact interrogation identity.
...
The dedicated National Phase 4 packaged gate passes twelve times across review hardening, including four consecutive runs of the final 32-row renderer after identity-bound rollback settlement and upload hardening, across the primary release-runtime history, residency, zero-I/O playback, quality-lock, memory, context-recovery, credit-release, exact-lookup, inspection-refresh/coalescing, post-finalization recovery receipt, active-playback failed-Site restoration, and successful Site-return criteria. `npm run verify` passes 285 frontend/script tests and 134 Rust tests across the library and binaries, together with the production build, formatting, warnings-denied clippy, Rust check, documentation links, and public-repository scan. The merged National Phase 2/3 packaged gates and unchanged selected-site Phase 4/5/6 packaged regressions also pass.
