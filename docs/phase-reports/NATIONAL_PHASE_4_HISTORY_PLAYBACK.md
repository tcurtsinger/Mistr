...
Two consecutive 3840 by 2160 results after review hardening:
...
- Rust store tests: chronology, staged rollback preservation, reversible post-eviction compensation, idempotent duplicate finalization, post-newer exact-cache release, one-oldest eviction at 20, and the same store/snapshot model at a non-shipping 30-frame limit.
...
The dedicated National Phase 4 packaged gate passes twice consecutively across the primary release-runtime history, residency, zero-I/O playback, quality-lock, memory, context-recovery, credit-release, exact-lookup, inspection-refresh/coalescing, post-finalization recovery receipt, active-playback failed-Site restoration, and successful Site-return criteria. `npm run verify` passes 272 frontend/script tests and 131 Rust tests across the library and binaries, together with the production build, formatting, warnings-denied clippy, Rust check, documentation links, and public-repository scan. The merged National Phase 2/3 packaged gates and unchanged selected-site Phase 4/5/6 packaged regressions also pass.
