...
- Playback tests: factor-1/factor-2 all-frame preparation, factor-4 fallback, projected-memory selection, cancellation cleanup, active camera re-preparation/supersession, recovery-time scrub waiting, source transition waiting for an in-flight selection fence, pause-only refinement, overview-camera behavior, and diagnostic 30-frame controller compatibility.
