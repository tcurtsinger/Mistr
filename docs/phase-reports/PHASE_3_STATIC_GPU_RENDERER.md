...
|---|---:|---:|---|
| Shared indexed radial grid | 42,229,704 bytes; 1,321,593 vertices; 7,914,240 indices; 2,638,080 triangles | 8.3 ms | No |
...
In all three, the same echoes remain geospatially fixed while the map moves/zooms, the dashed standard range layer renders below radar, standard anchor circles render above radar, and basemap roads/labels remain present. Public `map.addLayer`/`map.getLayer` calls verify all three diagnostic layers are installed in the intended insertion sequence. No private MapLibre member is read.
...
After 20 scripted pan/zoom positions plus normal basemap activity, the corrected dynamic renderer snapshot recorded 615 custom-layer draws.

| Measurement | Result |
|---|---:|
| Shader compile/link | 14.4 ms |
| Texture upload plus exact readback | 5.5 ms |
| First GPU-complete paint after upload | 6.5 ms |
| Custom-layer draw CPU P95 | 1.2 ms |
| Known renderer CPU model | 2,654,912 bytes (2.532 MiB) |
| Known radar GPU allocations | 2,650,224 bytes (2.527 MiB) |
