...
Each release carries a random 16-64 character acknowledgement ID. Rust retains a bounded window of 64 IDs and treats a repeated ID as success, so a lost control response can be retried without releasing a second buffer. A lease is marked released only after acknowledgement. Failed automatic stale/error acknowledgements remain queued and must flush before that client begins or requests more work; opening a replacement frontend session is the owner-loss recovery path.
