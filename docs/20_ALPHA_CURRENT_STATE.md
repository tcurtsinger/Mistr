...
- factor-4 playback and active scrubbing with no network, decode, IPC, or upload work, plus polling/backfill suspension while resident-only interaction owns the hot path;
...
- exact retained-observation point lookup bound to painted generation, time, content hash, and inspection identity, with a persistent inspection re-queried and cleared across every observation cut, one active expensive lookup, and only the newest pending receipt retained; and
...
The dedicated 3840x2160 Windows/WebView2 run retained 20 exact chronological NOAA MRMS observations spanning 38.12 minutes and kept all 20 complete factor-4 overviews GPU resident. It completed 1,000 direct resident transitions plus oldest/newest scrubs with zero network requests, response bytes, decoder runs, IPC transfers, IPC bytes, or texture uploads in the measured hot path. Peak National GPU allocation was 64,227,072 bytes, the maximum measured upload slice across initial staging, detail, and both recovery passes was 3.70 ms, and every provisional mutation was finalized before UI publication.
...
The Phase 4 branch also passes `npm run verify`: the public scan covered 270 candidate files, documentation links covered 81 Markdown files, all 45 frontend/script test files passed with 269 tests, the production TypeScript/Vite build passed, and all 129 Rust tests across the library and binary targets passed with formatting, warnings-denied clippy, and `cargo check`. Separate release-runtime reruns passed the merged National Phase 2 and Phase 3 gates plus the unchanged selected-site Phase 4, Phase 5, and two-pass Phase 6 gates.
