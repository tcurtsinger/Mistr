...
The active playback-chrome correction separates exact inspection request state from source-native data status. Each observation cut now shows a neutral busy `--.- dBZ` while the matching Rust lookup runs; it never uses `OUTSIDE RADAR COVERAGE` as a loading placeholder and never carries an old frame's value forward. Age and inspection share a fixed-width telemetry group, and playback-time assistive announcements remain stable. The focused packaged release gate sampled partial-backfill playback 88 times at 3840x2160 and 45 times each at 878x640 and 720x540: every sample was playing with an enabled transport, all pending samples had the correct busy presentation, no false coverage label appeared, and measured bar, timeline, telemetry, and sample-readout movement was 0 pixels. This focused pass does not replace or weaken the full Phase 4 upload/recovery threshold.
...
## National performance remediation (2026-08-04)

Owner-reported National symptoms — slow startup, full restaging on every pan/zoom, and repeated pixelate/sharpen cycles — were diagnosed in [National radar performance findings](29_NATIONAL_RADAR_PERFORMANCE_FINDINGS.md) and remediated on `claude/national-performance-fixes` (merged, PR #20): the selected observation now refines to a camera-independent complete-domain factor-1 presentation at any zoom, history commits preserve that detail instead of resetting to the overview, and GPU uploads share a measured 4 ms frame budget instead of one fixed 32-row band per vsync.

The owner's packaged retest showed the playing path still failed: camera changes stopped the loop for a full viewport re-preparation, supersession surfaced as a `could not change scans` notice, playback starved backfill, and home-view playback deleted the refined presentation. Round 2 (findings §0.1, `claude/national-playback-continuity`, merged PR #21) made playback motion-first: motion starts immediately, never stops for sharpness or a camera change, supersession never reaches the operator, and backfill/polling continue during playback.

Round 3 (findings §0.2, `claude/national-native-residency`) removes the two-level model entirely as an owner decision dated 2026-08-04: every retained observation is GPU-resident at the exact native grid (~1 GiB for the 20-frame loop; 1,280 MiB target / 1,536 MiB ceiling; backend target 2 GiB), the quality-preparation/refinement machinery is deleted from the product path, and blur is impossible by construction. National's supported-device floor is roughly 1.2 GiB of free GPU memory. Packaged phase-3 and phase-4 evidence PASSED on the validation machine on 2026-08-04 (findings §0.2).
