...
- transient predecessor prepare/transfer/GPU failures retain and retry the same unconsumed candidate with capped jitter/backoff before newer-only polling; if the Rust preparation completed but its IPC response was lost, predecessor/newer retry returns the exact stored stage without repeating acquisition;
...
Three passing 3840x2160 Windows/WebView2 runs across review hardening retained 20 exact chronological NOAA MRMS observations spanning 38.00 to 38.10 minutes and kept all 20 complete factor-4 overviews GPU resident; the final run followed the idempotent preparation-retry fix. Each completed 1,000 direct resident transitions plus oldest/newest scrubs with zero network requests, response bytes, decoder runs, IPC transfers, IPC bytes, or texture uploads in the measured hot path. The highest National GPU allocation was 65,201,668 bytes, the highest measured upload slice across initial staging, detail, and both recovery passes was 1.80 ms, and every provisional mutation was finalized before UI publication. The evidence runner holds one bounded resident reservation across timeline-sensitive assertions so a normal live MRMS rollover cannot make it compare different valid 20-frame histories.
...
The Phase 4 branch also passes `npm run verify`: the public scan covered 272 candidate files, documentation links covered 81 Markdown files, all 46 frontend/script test files passed with 272 tests, the production TypeScript/Vite build passed, and all 133 Rust tests across the library and binary targets passed with formatting, warnings-denied clippy, and `cargo check`. Separate release-runtime reruns passed the merged National Phase 2 and Phase 3 gates plus the unchanged selected-site Phase 4, Phase 5, and two-pass Phase 6 gates.
