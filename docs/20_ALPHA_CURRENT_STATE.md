...
Owner-reported National symptoms — slow startup, full restaging on every pan/zoom, and repeated pixelate/sharpen cycles — were diagnosed in [National radar performance findings](29_NATIONAL_RADAR_PERFORMANCE_FINDINGS.md) and remediated on `claude/national-performance-fixes`: the selected observation now refines to a camera-independent complete-domain factor-1 presentation at any zoom, history commits preserve that detail instead of resetting to the overview, and GPU uploads share a measured 4 ms frame budget instead of one fixed 32-row band per vsync. Playback's viewport-bounded sharp preparation and its zoom-6 gate are unchanged by memory necessity. Packaged before/after startup and pan/zoom evidence remains to be captured on the validation machine.
