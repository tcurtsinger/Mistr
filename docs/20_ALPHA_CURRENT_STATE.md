...
The owner's packaged retest showed the playing path still failed: camera changes stopped the loop for a full viewport re-preparation, supersession surfaced as a `could not change scans` notice, playback starved backfill, and home-view playback deleted the refined presentation. Round 2 (findings §0.1, `claude/national-playback-continuity`) makes playback motion-first as an owner decision dated 2026-08-04: motion starts immediately on the resident commons and never stops for sharpness or a camera change, quality preparation is prefetch-only beside the loop, supersession never reaches the operator, and backfill/polling continue during playback. Packaged before/after evidence remains to be captured on the validation machine.
