...
The dedicated 3840x2160 Windows/WebView2 run retained 20 exact chronological NOAA MRMS observations spanning 38.02 minutes and kept all 20 complete factor-4 overviews GPU resident. It completed 1,000 direct resident transitions plus oldest/newest scrubs with zero network requests, response bytes, decoder runs, IPC transfers, IPC bytes, or texture uploads in the measured hot path. Peak National GPU allocation was 64,227,072 bytes, the maximum measured upload slice across initial staging, detail, and both recovery passes was 2.80 ms, and every provisional mutation was finalized before UI publication.
