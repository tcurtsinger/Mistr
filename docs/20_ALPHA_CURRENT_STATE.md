...
**Merged baseline:** [PR #17 - National Phase 3 static MRMS renderer and source handoff](https://github.com/tcurtsinger/Mistr/pull/17), merge commit `be4b05b`

**Active change:** National Phase 4 bounded rolling history, all-frame overview residency, common-quality playback/scrubbing, paused detail refinement, and strictly newer polling; Ready-for-review PR pending from `codex/mistr-national-history`

**Branch:** `codex/mistr-national-history`
...
Mistr is the product. Merged `main` contains the qualified selected-site Alpha and the complete Phase 3 one-observation National CONUS source. The active Phase 4 branch adds 20 exact chronological National observations, current-first predecessor backfill, strictly newer polling, GPU-resident overview playback/scrubbing, and selected-frame detail refinement without automatic zoom handoff. This multi-observation behavior remains review-branch truth until merge. Velocity UI, alerts, cameras, video, notifications, broad settings, and the wider GustAVO feature set remain outside the product.
...
- Site and active-branch National direct timeline dragging scrub resident observations; there are no dedicated previous/next buttons. National enables play only after at least two complete overview observations are resident and truthfully labels partial backfill.
...
## Merged Phases 1 through 3, active Phase 4 history
...
The active Phase 4 branch adds:
...
- a provisional GPU/backend commit handshake with an identity-bound reversible Rust journal, so context loss, supersession, or backend failure restores both the prior chronology (including an evicted oldest frame) and the prior complete resident presentation, while a lost seal response may retry the last finalized identity idempotently and post-finalization context recovery must supply a fresh matching receipt before publication;
- factor-4 playback and active scrubbing with no network, decode, IPC, or upload work, plus polling/backfill suspension while resident-only interaction owns the hot path; direct scrub waits and its controls remain disabled until context recovery restores every common resident;
- exact selected-frame factor-1 refinement after pause/scrub settle, bounded detail prefetch for the adjacent temporal window, and common-quality fallback for every retained frame;
...
- exact retained-observation point lookup bound to painted generation, time, content hash, and inspection identity, with a persistent inspection re-queried and cleared across every observation cut, one active expensive lookup, and only the newest pending receipt retained;
...
### Active National Phase 4 evidence

Six passing 3840x2160 Windows/WebView2 runs across review hardening retained 20 exact chronological NOAA MRMS observations spanning 37.75 to 38.10 minutes and kept all 20 complete factor-4 overviews GPU resident; the latest run includes settled National playback before Site removal and resilient newer-observation polling. Each completed 1,000 direct resident transitions plus oldest/newest scrubs with zero network requests, response bytes, decoder runs, IPC transfers, IPC bytes, or texture uploads in the measured hot path. The highest National GPU allocation was 65,201,668 bytes, the highest measured upload slice across initial staging, detail, and both recovery passes was 2.00 ms, and every provisional mutation was finalized before UI publication. The evidence runner holds one bounded resident reservation across timeline-sensitive assertions so a normal live MRMS rollover cannot make it compare different valid 20-frame histories.

At high zoom, the selected observation and one adjacent temporal observation refined to exact factor-1 viewport chunks while the common factor-4 level remained available for the entire loop. Active playback returned to and locked factor 4 rather than alternating detailed and overview frames. A retained map inspection was re-queried at identical coordinates across newest-to-oldest-to-newest scrubs and accepted only matching observation identities; playback kept at most one Rust lookup active, coalesced waiting cuts to the newest receipt, and drained without pending work after pause. A real context reset advanced the receipt to epoch 2 and restored all 20 common residents from retained CPU bytes with zero backend activity. The packaged forced-failure scenario began with playback active, advanced from National generation 3 through failed Site generation 4 to restored National generation 5, forced another loss between renderer and backend finalization, accepted only the recovered receipt, proved playback stopped and backfill restarted, returned both shared transfer credits, and then safely handed the product back to KTLX Site. The dedicated command is `npm run test:national:phase4:packaged`; ignored evidence is written under `artifacts/national-phase-4/`.

The Phase 4 branch also passes `npm run verify`: the public scan covered 276 candidate files, documentation links covered 81 Markdown files, all 48 frontend/script test files passed with 280 tests, the production TypeScript/Vite build passed, and all 133 Rust tests across the library and binary targets passed with formatting, warnings-denied clippy, and `cargo check`. Deterministic tests prove complete resident counts remain blocked until recovery is actually painted and fenced, finalization continues beyond three failures, a matching sealed snapshot resolves a lost response, source handoff awaits an in-flight selection, healthy no-newer polling clears stale errors, and delay-command failure falls back without stopping polling. Separate release-runtime reruns passed the merged National Phase 2 and Phase 3 gates plus the unchanged selected-site Phase 4, Phase 5, and two-pass Phase 6 gates.
...
Complete and review National Phase 4 only. After its Ready-for-review PR passes review, the owner merges it and separately authorizes Phase 5. Do not begin the Phase 5 long-session, rollover, device-floor, installer, sleep/wake, or clean-machine matrix on this branch. Microsoft Store packaging and signing remain paused.
