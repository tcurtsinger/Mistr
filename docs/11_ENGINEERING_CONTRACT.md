...
Merged Phase 4 exposes up to 20 exact National timeline observations. It enables playback only after two complete common-level observations are resident, uses direct scrubbing without separate previous/next controls, and names partial backfill truthfully. Playback is motion-first (owner decision, 2026-08-04): motion begins immediately at the always-resident complete common level and is never held or stopped for sharpness or a camera change. A bounded all-frame factor-1/factor-2 preparation runs beside the loop as prefetch-only residency work, upgrades the quality lock between frames when complete, and is superseded silently by a newer camera. Frame selection itself remains resident-only: no acquisition, grid decode, bulk IPC, or upload ever gates a frame advance. Backfill and strictly-newer polling continue during playback; each history commit briefly pauses and resumes the loop through the existing replacement contract, and sharp preparation defers until retained history settles. Hidden `window.__MISTR_NATIONAL_PHASE2__`, `window.__MISTR_NATIONAL_PHASE3__`, and `window.__MISTR_NATIONAL_PHASE4__` evidence APIs do not become normal controls.
