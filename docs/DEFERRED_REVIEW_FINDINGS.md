...
- **Reason:** Product-foundation runs occasionally exceeded the stabilized-heap-growth threshold while retaining zero long tasks, zero hot-path work, normal timing, and bounded residency. The heavier rolling-history lifecycle repeated the measurement twice on the same release executable, satisfying the prior investigation trigger. The runner had taken one DevTools garbage-collection pass and sampled 500 ms later; it now performs a second explicit collection and settling interval before sampling. No threshold changed. The same unchanged executable then passed with stabilized samples of 85,095,638 and 87,938,245 bytes while all rolling, recovery, and hot-path gates also passed. This demonstrates that the earlier single-collection sample was not reliably stabilized, but it does not prove that every future heap-growth failure is measurement-only.
- **Revisit:** If the strengthened two-collection gate fails twice on the same source state or reproduces on a second Windows machine, retain all per-scenario heap samples and investigate application retention before changing the measurement or threshold.
