...
- An `RGB32F` radial-metadata texture stores measured azimuth, half-beam width, and elevation in radians.
...
This is not literally “one scan equals one GPU buffer.” One observation is a compact resource set: raw texture, status texture, azimuth lookup, radial metadata, palette, and a six-vertex quad. Phase 4 may share compatible immutable resources, but correctness does not depend on that optimization.
...
- The CPU builds the bounded azimuth lookup from those beam widths. The lookup nominates a radial candidate; CPU interrogation and the shader then check the exact bearing against that radial's measured half-beam width.
- Lookup value zero and exact beam-check failures are transparent missing coverage. Quantized lookup bins cannot bridge or widen an unmeasured radial gap.
...
It built in 8.9 ms in the final corrected packaged run, but repeats gate-boundary geometry and introduces index/edge rules that the polar shader does not need. It remains a viable future reference renderer, not the selected production candidate.
...
| Known GPU radar allocations | 2,655,984 bytes (2.533 MiB) |
| Projected 20-observation CPU model | 50.64 MiB |
| Projected 20-observation GPU allocations | 50.66 MiB |
| Shader compile/link | 1.7 ms |
| Upload plus exact texture readback | 5.8 ms |
| GPU-completion first-paint receipt | 6.6 ms |
| Custom-layer draw CPU P95 | 1.1 ms across 543 draws |

The one-observation resource count includes two 1,319,040-byte gate textures, one 8,192-byte azimuth lookup, one 8,640-byte radial-metadata texture, one 1,024-byte palette, and a 48-byte quad. It excludes opaque driver allocation/alignment and MapLibre/basemap resources, so it is a known-allocation estimate rather than total GPU-process memory.
...
- **Irregular radial gaps:** the zero-valued lookup plus an exact post-lookup beam-width check are tested and discard unmeasured bearings, including partially overlapping lookup bins.
