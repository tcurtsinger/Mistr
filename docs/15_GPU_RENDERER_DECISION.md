...
- The shader uses the matrix supplied through MapLibre's public `CustomRenderMethodInput.defaultProjectionData.mainMatrix`. It does not read private style, transform, painter, or source internals.
...
It built in 8.9 ms in the recorded packaged run, but repeats gate-boundary geometry and introduces index/edge rules that the polar shader does not need. It remains a viable future reference renderer, not the selected production candidate.
...
| Renderer CPU model | 2,652,032 bytes (2.529 MiB) |
| Known GPU radar allocations | 2,647,344 bytes (2.525 MiB) |
| Projected 20-observation CPU model | 50.58 MiB |
| Projected 20-observation GPU allocations | 50.49 MiB |
| Shader compile/link | 1.7 ms |
| Upload plus exact texture readback | 5.8 ms |
| GPU-completion first-paint receipt | 6.4 ms |
| Custom-layer draw CPU P95 | 1.0 ms across 817 draws |

The one-observation resource count includes two 1,319,040-byte gate textures, one 8,192-byte azimuth lookup, one 1,024-byte palette, and a 48-byte quad. It excludes opaque driver allocation/alignment and MapLibre/basemap resources, so it is a known-allocation estimate rather than total GPU-process memory.

Ten CPU-generated gate anchors at near and 117.125 km ranges recovered the exact source radial/gate. Maximum round-trip error was `5.851e-9` m in range and `1.951e-10` degrees in bearing. A packaged WebGL framebuffer readback exactly recovered raw code 67, status 0, encoded radial 1, and premultiplied RGBA `[30, 106, 114, 136]` from the uploaded textures.
...
The packaged WebView2 device was hardware ANGLE/D3D11 on an NVIDIA GeForce RTX 4080. WebGL2 reported `MAX_TEXTURE_SIZE = 16384` and 16 fragment texture units, above the required 4,096 lookup width and four samplers.
