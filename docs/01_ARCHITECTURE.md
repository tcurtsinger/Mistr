...
- A Site transition requested from National waits for the currently observed National acquisition/history transaction to settle before it advances the shared transfer generation or cancels the National working set. It then revalidates that the same Site transition still owns source intent.
