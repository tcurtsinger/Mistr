...
| R44 | GPU and backend chronology diverge across commit, context loss, or supersession | Medium | Critical | UI/renderer names a frame absent from retained history, or Rust evicts a frame that the renderer restored | Provisional fence receipt; retain prior resource graph; identity-bound reversible Rust commit journal including evicted ownership; renderer finalize then backend seal; compensating rollback tests | Mitigated by working-set and reversible-eviction tests plus packaged zero-uncommitted state |
...
| R46 | Failed National-to-Site replacement leaves the visible National loop inactive | Medium | Critical | Old National paint remains, but its history generation was cancelled and cannot poll or backfill | Current Site failure callback runs after coordinator rollback; if National is still painted and the Site generation owns transfer state, start a newer National session without removing the old paint; packaged forced-failure proof binds coordinator, backend, renderer, and transfer generations | Mitigated by session unit tests and the Phase 4 packaged failed-Site recovery scenario |
