...
| R44 | GPU history changes before backend chronology commits | Medium | Critical | UI/renderer names a frame absent from retained history after failure or supersession | Provisional fence receipt; retain prior resource graph; backend generation check; explicit finalize/rollback transaction | Mitigated by working-set rollback tests and packaged zero-uncommitted state |
