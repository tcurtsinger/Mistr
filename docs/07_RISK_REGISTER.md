...
| R44 | GPU and backend chronology diverge across commit, context loss, supersession, or a lost finalization response | Medium | Critical | UI/renderer names a frame absent from retained history, Rust evicts a frame that the renderer restored, or the renderer finalizes but a repeated backend seal is rejected | Provisional fence receipt; retain prior resource graph; identity-bound reversible Rust commit journal including evicted ownership; renderer finalize then backend seal; retain one bounded last-sealed identity for idempotent retry; compensating rollback tests | Mitigated by working-set, reversible-eviction, and duplicate-finalization tests plus packaged zero-uncommitted state |
...
| R46 | Failed National-to-Site replacement leaves the visible National loop inactive or collides with old playback | Medium | Critical | Old National paint remains but its history generation cannot poll/backfill, or an old selection waiter blocks replacement staging | Current Site failure callback runs after coordinator rollback; pause and await the existing playback/working-set operations; if National is still painted and the failed Site generation still owns transfer state, start a newer National session without removing the old paint; packaged forced-failure proof begins with playback active and binds coordinator, backend, renderer, and transfer generations | Mitigated by session unit tests and the Phase 4 packaged active-playback failed-Site recovery scenario |
