...
Each Phase 4 history mutation has one staged backend identity and one provisional GPU mutation. The backend commits only after complete chunk upload, viewport draw, and GPU fence; a superseded or failed mutation rolls both sides back to the prior complete frame. Point lookup re-decodes the exact retained compressed object under a single-operation gate when the selected base grid is not cached. Downloaded objects remain memory-owned and ignored artifacts rather than a directory-scanned provider cache.
