...
9. A backend commit remains reversibly journaled until renderer finalization; simulated post-fence cancellation/context loss restores an evicted oldest frame and the prior GPU graph before either side can publish. After renderer finalization, frontend sealing must continue beyond three local IPC failures with capped delay until the exact journal resolves; an identity-matching non-reversible snapshot may recover a lost successful response, and repeating the same finalization remains idempotent.
