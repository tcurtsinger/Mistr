...
3. Playback tests prove play and direct scrub select factor 4, direct scrub waits while common residency is incomplete after context loss, and complete texture counts cannot release that barrier until the renderer has repainted, fenced, published a receipt, and left provisional state. Fine refinement waits for pause/settle, overview camera changes do not create a refinement loop, and the same controller accepts the diagnostic 30-frame cap.
...
8. Exact point lookup must name one retained painted identity; a late identity result is rejected, and a persistent map inspection is re-queried after every playback or scrub observation cut before a new value appears. Deterministic queue tests prove only one expensive lookup runs at a time and multiple waiting receipts collapse to the newest; packaged playback requires the queue to drain with no pending work after pause.
9. A backend commit remains reversibly journaled until renderer finalization; simulated post-fence cancellation/context loss restores an evicted oldest frame and the prior GPU graph before either side can publish. Before renderer finalization, identity-bound rollback must continue beyond three ambiguous local IPC failures until Rust responds or an exact snapshot proves that staged/reversible state and the candidate are gone. After renderer finalization, frontend sealing follows the equivalent unbounded-attempt/capped-delay contract; an identity-matching non-reversible snapshot may recover a lost successful response, and repeating the same finalization remains idempotent.
