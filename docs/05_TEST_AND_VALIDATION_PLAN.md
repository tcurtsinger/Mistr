...
3. Playback tests prove factor-1/factor-2 all-frame viewport preparation, complete overview backing behind either detail factor, factor-4 fallback with obsolete-detail pruning, cancellation cleanup, and a uniform quality lock. Direct scrub waits while common residency is incomplete after context loss, and complete texture counts cannot release that barrier until the renderer has repainted, fenced, published a receipt, and left provisional state. Fine refinement waits for pause/settle, user-cancelled quality preparation defers its retry until the resident-only activity is released, an active camera move received before or during staging cancels and re-prepares before resuming, overview camera changes do not create a refinement loop, and the same controller accepts the diagnostic 30-frame cap.
...
24. A dedicated partial-backfill playback-chrome gate samples the packaged bar while retained observations and exact point results change. At 3840x2160 and compact 878x640/720x540 viewports it requires Play to remain enabled, pending lookup text to remain neutral and busy, zero false outside-coverage labels, one stable live announcement, and no measurable bar, timeline, telemetry, or sample-readout geometry movement.

The full dedicated command is `npm run test:national:phase4:packaged`; the focused playback-chrome command is `npm run test:national:playback-chrome:packaged`. Reports and screenshots are generated only under ignored `artifacts/national-phase-4/`. Phase 4 exit also requires `npm run verify`, documentation/public/whitespace checks, the merged National Phase 2 and 3 packaged gates, and unchanged selected-site Phase 4, 5, and 6 packaged regressions. Hidden `window.__MISTR_NATIONAL_PHASE4__` is evidence-only; all earlier diagnostic APIs remain required.
