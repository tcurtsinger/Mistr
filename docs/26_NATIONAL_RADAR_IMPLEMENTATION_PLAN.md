...
When play begins, the controller chooses the finest complete presentation level that covers the current viewport for every retained observation in the playable loop. At regional zoom it may briefly prepare that all-frame coverage before motion begins: factor 1 first, factor 2 if the projected allocation would exceed the 200 MiB target, and factor 4 as the always-complete fallback. The old complete frame remains visible and preparation can be cancelled. The selected level becomes the playback quality lock; every frame then paints at that same level and normal cadence without acquisition, grid decode, bulk IPC, or upload work. If the camera changes during playback, Mistr cancels the old camera request, prepares one complete all-frame level for the new coverage, and resumes only after it is ready.
...
Holding the previous frame is reserved for incomplete initial backfill, a cancelled working-set mutation, a source/generation transition, or the bounded preparation interval before a regional all-frame quality lock. Once playback starts, missing fine detail never stalls an individual frame or permits mixed quality.
...
**Merged status:** Phase 4 merged through PR #18. A focused post-merge visual-fidelity correction prepares complete regional all-frame detail before playback without beginning Phase 5.
...
Exit gate: all 20 retained observations play and scrub entirely from GPU residency at the normal CONUS overview with bounded memory and zero hot-path acquisition, decode, disk, IPC, or upload activity. At regional zoom, bounded preparation may extend one camera-appropriate level across all frames before play begins; after that lock, every frame maintains normal cadence, consistent quality, and zero acquisition/grid-decode/bulk-IPC/upload work. The prior frame may be held during this explicit preparation or when the target lacks the common level itself.

Twelve passing Phase 4 packaged runs across review hardening at 3840 by 2160, including four consecutive runs of the final 32-row renderer after identity-bound rollback settlement and upload hardening, retain 20 exact chronological observations spanning 37.70 to 38.10 minutes, keep every complete factor-4 overview resident, complete 1,000 transitions plus oldest/newest direct scrubs with zero network/decode/IPC/upload activity, and record at most 65,201,668 bytes of GPU allocation with a 2.20 ms maximum passing upload slice across initial staging, detail, and both recovery passes. A post-merge visual-fidelity reproduction then prepared all 20 regional frames at exact factor 1 before playback, held one common lock, peaked at 86,060,064 GPU bytes, and performed no acquisition, grid decoding, bulk IPC, or texture uploads after motion began; camera supersession tests require re-preparation before resume. The complete factor-4 overview remains the rollback and country-view level. Persistent inspection refresh remains identity-bound through its separate latest-only point-lookup path. Real context loss restores all 20 common overviews from retained CPU bytes, both shared credits return, and source failure still preserves the last complete paint before safe Site handoff. Complete evidence is in [National Phase 4 History and Playback](phase-reports/NATIONAL_PHASE_4_HISTORY_PLAYBACK.md).
