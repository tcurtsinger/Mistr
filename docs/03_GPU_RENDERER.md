...
Render measured radar data inside MapLibre without representing observations as provider-colored raster tiles. The basemap remains ordinary MapLibre content; selected-site radar uses its qualified polar WebGL2 layer, and National uses a separate numeric-grid custom layer rather than pretending MRMS is a polar sweep or a MapLibre radar-tile animation.

Merged Phases 1 through 4 leave the polar Site renderer unchanged while supplying source coordination, validated numeric chunks, `NationalGridLayer`, and bounded per-observation common/detail residency; National shares neither polar textures nor independent transfer credits with Site.
...
`NationalGridLayer` uploads one `R16UI` chunk lease at a time. Each texture is allocated and filled in bounded 32-row bands over up to nine animation frames, with a hard configured upload slice no greater than 4 ms. It keeps active and staged presentations separate; a receipt fires only when every coverage-required chunk exists, all chunks draw, and a matching GPU fence completes. Receipts identify observation/generation/hash, presentation factor, coverage version and kind, required chunk count, context epoch, framebuffer, staging duration, maximum upload slice, and uploaded bytes.

At the normal CONUS view, factor 4 is a complete 28-chunk presentation. Phase 4 keeps this common level resident for every retained observation. At regional zoom, play preparation attempts factor-1 viewport coverage for all retained observations, measures the projected total allocation, and falls back to factor 2 when factor 1 would exceed the 200 MiB target. Factor 4 remains the complete-domain fallback. Playback begins only after one factor is complete for the entire loop and then changes only selection/draw state; it never substitutes individually refined frames. The prior complete paint remains visible while preparation stages, and cancellation removes partial detail.

Each history append/backfill is a provisional renderer mutation after its GPU fence: retired resources remain owned while Rust applies the matching chronology change with a reversible journal. Before renderer finalization, an ambiguous commit/rollback response keeps the identity-bound rollback alive until Rust responds or a snapshot proves that the candidate and all provisional state are gone. Renderer finalization deletes the prior GPU resources and then Rust seals the journal; after that irreversible point, capped-delay retries continue without a fixed attempt limit until the matching Rust journal is sealed, while an identity-matching non-reversible snapshot can recover a lost successful response. The last same-identity seal may also be repeated idempotently. Context loss, supersession, or backend failure before renderer finalization restores and repaints the prior residency graph while Rust restores the prior chronology. If context loss instead lands after renderer finalization while the backend seal is in flight, publication waits for the new-context receipt with the same generation, observation, content, presentation, coverage, and chunk-count identity. All resident, staged, provisional-retired, reversible-commit, palette, and geometry bytes are included in the bounded ledgers and 256 MiB hard ceiling; reversible-commit bytes include a prior detailed frame whenever replacement makes that frame uniquely journal-owned. Twelve packaged 20-frame runs across review hardening measured at most 65,201,668 bytes and a 2.20 ms maximum passing upload slice across initial staging, detail, and both recovery passes. During the final review fix, 96- and 64-row recovery bands separately exposed 4.40 and 5.50 ms rejected slices; the threshold was not relaxed. The unchanged final 32-row renderer then passed four consecutive packaged runs at 1.70, 1.90, 1.80, and 2.10 ms, including the latest full source. The existing non-shipping factor-4 ledger remains 96,243,964 bytes for 30 frames plus staging without changing the renderer/schema model.

Native uses nearest selected-level numeric sampling. Smooth applies bilinear interpolation only when all four contributing cells are valid; otherwise it falls back to the nearest valid cell and never blends across missing or no-coverage status. Both use the same selected presentation level and premultiplied reflectivity palette and cannot change exact Rust-owned interrogation. At extreme zoom, factor 1 exposes NOAA MRMS's approximately one-kilometre source cells; Smooth may filter their edges but cannot recreate finer local Level II structure.

Paused camera moves reuse the matching Rust-owned prepared detail frame when generation, observation time, content hash, and presentation factor are unchanged. Only viewport chunk selection and transfer repeat; Mistr does not gzip-decode and encode the full CONUS factor-1 frame again merely because the camera panned.

On real context loss, CPU chunk arrays for the selected presentation, its fallback, and every common resident remain authoritative. The layer discards stale WebGL handles, restores the prior complete presentation if a replacement was in flight, and rehydrates selected/fallback chunks before the remainder through the same row-banded animation-frame uploads under the 4 ms ceiling. It issues a new context-epoch receipt after the selected presentation draws; playback and direct scrub wait until all common textures are resident, the renderer is painted, the recovered receipt exists, and no provisional mutation remains. Complete texture counts before the final repaint/fence do not release the barrier. Product controls stay disabled and recovery remains the visible status. This wait performs no network, disk, decode, or IPC work.
