...
Merged Phases 1 through 3 leave the polar Site renderer unchanged while supplying source coordination, validated numeric chunks, and `NationalGridLayer`. The active Phase 4 branch extends that layer to bounded per-observation common/detail residency; it shares neither polar textures nor independent transfer credits with Site.
...
At the normal CONUS view, factor 4 is a complete 28-chunk presentation. Phase 4 keeps this common level resident for every retained observation, so play and direct scrub change only selection/draw state. At high zoom, the selected observation may retain a factor-1 viewport plus factor-4 fallback, with bounded adjacent temporal detail prefetched only while paused. Active play and scrub select factor 4 for every observation and never substitute individually refined frames.
