...
Each history append/backfill is a provisional renderer mutation after its GPU fence: retired resources remain owned while Rust applies the matching chronology change with a reversible journal. Renderer finalization deletes the prior GPU resources and then Rust seals the journal; the last same-identity seal may be repeated idempotently if its IPC response is lost. Context loss, supersession, or backend failure before renderer finalization restores and repaints the prior residency graph while Rust restores the prior chronology. All resident, staged, provisional-retired, reversible-commit, palette, and geometry bytes are included in the bounded ledgers and 256 MiB hard ceiling. The packaged 20-frame run measured a 63,694,560-byte peak and 1.70 ms maximum upload slice, below the 200 MiB target and 4 ms frame budget. The existing non-shipping factor-4 ledger remains 96,243,964 bytes for 30 frames plus staging without changing the renderer/schema model.
