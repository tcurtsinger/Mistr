...
Each history append/backfill is a provisional renderer mutation after its GPU fence: retired resources remain owned while Rust applies the matching chronology change with a reversible journal. Renderer finalization deletes the prior GPU resources and then Rust seals the journal; the last same-identity seal may be repeated idempotently if its IPC response is lost. Context loss, supersession, or backend failure before renderer finalization restores and repaints the prior residency graph while Rust restores the prior chronology. If context loss instead lands after renderer finalization while the backend seal is in flight, publication waits for the new-context receipt with the same generation, observation, content, presentation, coverage, and chunk-count identity. All resident, staged, provisional-retired, reversible-commit, palette, and geometry bytes are included in the bounded ledgers and 256 MiB hard ceiling. Three packaged 20-frame runs across review hardening measured at most 65,201,668 bytes and a 1.80 ms maximum upload slice across initial staging, detail, and both recovery passes, below the 200 MiB target and 4 ms frame budget. The existing non-shipping factor-4 ledger remains 96,243,964 bytes for 30 frames plus staging without changing the renderer/schema model.
