...
Each history append/backfill is a provisional renderer mutation after its GPU fence: retired resources remain owned until the Rust chronology commit succeeds. Finalization then deletes them; supersession or backend failure restores and repaints the prior residency graph. All resident, staged, provisional-retired, palette, and geometry bytes are included in the 256 MiB hard-ceiling check. The packaged 20-frame run measured a 65,292,096-byte peak and 1.30 ms maximum upload slice, below the 200 MiB target and 4 ms frame budget. The existing non-shipping factor-4 ledger remains 96,243,964 bytes for 30 frames plus staging without changing the renderer/schema model.
