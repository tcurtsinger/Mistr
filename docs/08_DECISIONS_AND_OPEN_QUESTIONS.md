...
**Resolved for the resident-loop baseline:** Retain compact raw codes, detailed statuses, radial metadata, and azimuth lookup for every resident observation; release the packed float-value transfer after upload. The current 20-observation projection is about 50.58 MiB CPU and 50.49 MiB known GPU allocations. Phase 4 must confirm actual residency and revise visibly if another product/layout exceeds budget.
