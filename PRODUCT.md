...
National radar is the approved next product milestone. Merged Phases 1 and 2 established the typed source coordinator plus strict NOAA MRMS acquisition, numeric decoding, value-aware levels, bounded retention, and `PackedGrid v1` transport. The active Phase 3 review branch adds the first usable static National source: an explicit `National`/`Site` choice, one current CONUS observation, a separate numeric-grid WebGL2 renderer, complete-coverage GPU receipts, exact Rust-owned point interrogation, and atomic source handoff.

This Phase 3 behavior is implemented and packaged-validated on `codex/mistr-national-static-renderer`, but it is not part of shipped `main` until its pull request is reviewed and merged. National history, polling, and playback remain Phase 4 work; the Phase 3 National timeline deliberately contains one exact current observation. The approved phased product and engineering contract is [National Radar Implementation Plan](docs/26_NATIONAL_RADAR_IMPLEMENTATION_PLAN.md).
