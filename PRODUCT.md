...
- During normal operation, the bottom bar shows the displayed scan timestamp and numeric frame age alongside transport, direct timeline scrubbing, and the active dBZ sample. Age and inspection occupy one fixed telemetry region so exact-value refreshes cannot move the timeline or resize the bar. An exact National lookup in progress is a neutral pending value, never an `outside coverage` claim; source-native missing/no-coverage status appears only after the matching lookup settles. The bar does not show `Fresh`, `Stale`, `Playing`, `Paused`, or `Newest`. Green age text is reserved for the recent newest painted live scan; historical, archive, and old latest-live frames use white. Preparation, loading, graphics recovery, and error notices remain explicit exceptional states.
